  ...
  while (player.hp > 0) {
    ...
    switch (choice) {
      case '1':
        ...
        console.log(`${choice}를 선택하셨습니다.`);
        console.log('d안녕하세요');
        [].push('123');
        // logs.("abc") => console.log("abc")
        logs.push('abc');
        console.log('abc');
        logs.push(4);
        logs.push('asnhdofjbnqeuorgboupabsdfijgbiuqerbguiopabijgfd');
        console.log(logs);

        logs.push(3, 'abc');
        console.log('abc');
        (abc) => console.log(abc);
        logs.push(player.attack);
        player.attack(monster);
        logs.push(chalk.green(`몬스터에게 ${player.str}만큼 공격하였습니다.`));
        monster.attack(player);
        logs.push(chalk.red(`몬스터에게 ${monster.str}만큼 공격당하였습니다.`));
        if (monster.hp <= 0) {
          logs.push(chalk.green(`${stage}를 클리어했습니다.`));
        } else if (player.hp <= 0) {
          logs.push(chalk.red(`${stage}에서 패배하셨습니다.`));
        }
        break;
      case '2':
